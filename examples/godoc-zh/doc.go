...
Godoc-zh extracts and generates documentation for Go programs.
...
When godoc-zh runs as a web server, it creates a search index from all .go files
...
If the sync exit status is 1, godoc-zh assumes that it succeeded without errors
but that no files changed; the index is not updated in this case.

In all other cases, sync is assumed to have failed and godoc-zh backs off running
